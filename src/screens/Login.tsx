...
import { ScrollView, View, Text, TextInput, Pressable, StyleSheet, Image, Alert, ActivityIndicator } from 'react-native';
...
export default function Login() {
  ...
  const [email, setEmail] = React.useState(__DEV__ ? 'taylor@example.com' : '');
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: 16, backgroundColor: theme.colors.surface }}>
      <Image
        ...
        style={{ width: 160, height: 160, borderRadius: 12, alignSelf: 'center', marginTop: 8 }}
      />
      <Text style={[s.h, { color: theme.colors.primary, alignSelf: 'center', marginTop: 8 }]}>Sign in</Text>
      ...
      <View style={s.card}>
        ...
        <Pressable style={[s.btn, { borderColor: '#333' }]} onPress={() => nav.navigate('PhoneSignIn')} disabled={loading}>
          <Text style={[s.bt, { color: '#333' }]}>Sign in with Phone (SMS)</Text>
        </Pressable>
      </View>
    </ScrollView>
  );
