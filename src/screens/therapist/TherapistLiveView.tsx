...
export default function TherapistLiveView() {
  ...
  const snap = async () => {
    const ts = Date.now();
    try {
      const path = `artifacts/${studentId}/${ts}/notes.txt`;
      await storage().ref(path).putString(notes || '');
      const url = await storage().ref(path).getDownloadURL();
      await backend.saveArtifact({
        id: String(ts),
        ts,
        studentId,
        payload: { type: 'snapshot', notes, url },
      });
      Alert.alert('Saved', 'Snapshot saved (with Cloud Storage URL).');
    } catch (e: any) {
  ...
  React.useEffect(() => {
    backend.getArtifactsByStudent(studentId).then(setArtifacts);
  }, [studentId]);
  ...
  return (
    <MFAGate>
      <ScrollView contentContainerStyle={{ padding: 16, backgroundColor: theme.colors.surface }}>
        ...
        <Pressable style={[s.btn, { borderColor: theme.colors.primary }]} onPress={snap}>
          <Text style={[s.bt, { color: theme.colors.primary }]}>📸 Snapshot</Text>
        </Pressable>

        <Text style={[{ marginTop: 10, fontWeight: '900', color: theme.colors.primary }]}>
          Recent Artifacts
        </Text>
        {artifacts.map((a, i) => (
          <Text key={i} style={{ marginTop: 4 }}>
            {new Date(a.ts).toLocaleString()} — {JSON.stringify(a.payload)}
          </Text>
        ))}
      </ScrollView>
    </MFAGate>
...
const s = StyleSheet.create({
  ...
  btn: { borderWidth: 2, borderRadius: 10, padding: 10, marginTop: 8, alignSelf: 'flex-start' },
  bt: { fontWeight: '900' },
});
